...
def expected_goals(
    ...
    *,
    ...
    λ = exp(α[home] + β[away] + γ · is_home), μ = exp(α[away] + β[home]).
    `is_home = 0` se `is_neutral=True`, 1 altrimenti.
    """
    a_h = model.alpha_of(home)
    b_h = model.beta_of(home)
    a_a = model.alpha_of(away)
    b_a = model.beta_of(away)
    is_home_adv = 0.0 if is_neutral else 1.0
    lam = float(np.exp(a_h + b_a + model.gamma * is_home_adv))
...
def match_outcome_90(
    ...
    *,
    ...
    Ritorna `Outcome90` con `(p_home_win, p_draw, p_away_win)` che sommano a 1, più
    i gol attesi λ, μ riusabili in #7 per i supplementari.
    """
    lam, mu = expected_goals(model, home, away, is_neutral=is_neutral)
